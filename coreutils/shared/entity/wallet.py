...
FINANCE_ACCOUNT_WALLET_MAPPING: MappingProxyType[FinanceAccountType, Sequence[WalletType]] = MappingProxyType(
    {
...
class Wallet:
    ...
    def calculate_client_balance(wallet: Wallet) -> Decimal:
        return Decimal(
            sum(asset.amount_usd for asset in wallet.assets.values() if asset.type not in wallet.secondary_assets)
        )
    ...
    def apply_transfer_delta(self, *, transfer: Transfer) -> None:
        ...
        if current is None:
            if transfer.amount < 0:
                raise ValueError(f"Negative balance for {transfer.type}: {transfer.amount}")
